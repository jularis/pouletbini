...
trait SupportTicketManager
{
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        $adminNotification->title     = 'Un nouveau ticket a été ouvert';
        ...
        $notify[] = ['success', 'Ticket ouvert avec succès!'];
    ...
    public function viewTicket($ticket)
    {
        ...
        return view($this->activeTemplate . $this->userType . '.support.view', compact('myTicket', 'messages', 'pageTitle', 'user'));
    }
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        $notify[] = ['success', 'Le Service Support a repondu avec succès!'];
    ...
    protected function storeSupportAttachments($messageId)
    {
        ...
        foreach ($this->files as $file) {
            try {
                ...
                $notify[] = ['error', 'Le fichier ne peut pas être chargé'];
                return $notify;
            }
        }
                ...
                function ($attribute, $value, $fail) {
                    ...
                    foreach ($this->files as $file) {
                        ...
                        if (($file->getSize() / 1000000) > $this->maxSize) {
                            return $fail("Maximum $this->maxSize MB de taille de fichier autorisée !");
                        }

                        if (!in_array($ext, $this->allowedExtension)) {
                            return $fail("Seuls les fichiers png, jpg, jpeg, pdf, doc, docx sont autorisés.");
                        }
                    }

                    if (count($this->files) > 5) {
                        return $fail("Un maximum de 5 fichiers peut être chargé");
                    }
                },
            ],
    ...
    public function closeTicket($id)
    {
        ...
        $ticket->save();
        $notify[] = ['success', 'Le ticket de support a été fermé avec succès!'];
        return back()->withNotify($notify);
    }
