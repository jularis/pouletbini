            ...
            <form action="{{route('manager.livraison.store')}}" method="POST" id="flocal">
                <div class="card-body">
                    ...
                    <div class="row mb-30">
                        ...
                        <label class="control-label">@lang('Selectionner un numero de Bande')</label>
                                <select class="form-control select2-multi-select float-end" name="bande[]" id="bande" multiple required> 
                                                @foreach($bandes as $bande)
                                                <option value="{{$bande->id}}" @selected(in_array(@$bande->id, @request()->bande ?? array()))>{{__($bande->numero_bande)}}</option>
                                                @endforeach
                                            </select>
                                            </div>
                    </div>
